...
export default function EventsContainer({ list }: { list: EventsType }) {
  ...
  const defaultTabIndex = upcomingEvents.length > 0 ? 0 : 1;
  return (
        ...
        <div>
          <TabPanel index={0}>
            {upcomingEvents.length > 0 ? (
              upcomingEvents.map((event) => <EventCard key={event.regLink} {...event} type="UPCOMING" />)
            ) : (
          ...
          <TabPanel index={1}>
            {pastEvents &&
              pastEvents.map((event) => <EventCard key={event.regLink} {...event} type="PAST" />)}
          </TabPanel>
        </div>
      </Tabs>
    </div>
